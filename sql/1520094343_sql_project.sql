...
Ans: SELECT COUNT(membercost) FROM `Facilities` WHERE membercost = 0;
...
Ans: SELECT facid, name, membercost, monthlymaintenance FROM Facilities WHERE membercost < monthlymaintenance * 0.2 LIMIT 0 , 30;
...
Ans: SELECT DISTINCT CONCAT( joined.firstname,  ' ', joined.surname ) AS full_name, joined.fac_name
...
Ans: SELECT mems.surname AS member, facs.name AS facility,
CASE
WHEN mems.memid = 0
...
Ans: SELECT member, facility, cost
FROM (
